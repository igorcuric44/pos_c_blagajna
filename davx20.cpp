...
case WM_NOTIFY:
		{
if(((LPNMHDR)lParam)->code == NM_DBLCLK && LOWORD(wParam) ==HSKLADISTE)   
                 {

int iSlected=0;
				  int j=0;

				  iSlected=SendMessage(hskladiste,LVM_GETNEXTITEM,-1,LVNI_FOCUSED);
				  ...
				  SendMessage(hskladiste,LVM_GETITEMTEXT, iSlected, (LPARAM)&lvi);
...
SendMessage(hskladiste,LVM_INSERTCOLUMN,5,(LPARAM)&LvCol); // ...same as above
...
lvi.pszText=Temp;
SendMessage(hskladiste,LVM_SETITEM,0,(LPARAM)&lvi); // Enter text to SubItems
}
...
SendMessage(hskladiste,LVM_INSERTITEM,0,(LPARAM)&lvi); // Send to the Listview
				...
				for(int i=1;i<=5;i++) // Add SubItems in a loop
				{
					...
					sprintf(Temp,"SubItem %d",i);
					lvi.pszText=Temp;
					SendMessage(hskladiste,LVM_SETITEM,0,(LPARAM)&lvi); // Enter etxt to SubItems
				}
