...
hskladiste1= CreateWindowEx(LVS_EX_FULLROWSELECT,WC_LISTVIEW, 0,WS_VISIBLE|WS_BORDER|WS_CHILD | LVS_REPORT ,450, 10, 300, 300,hwnd,(HMENU)hsSKLADISTE1,hins,NULL);
...
if(HIWORD(wParam) == BN_CLICKED && LOWORD(wParam) ==hsSKLADISTE1)   
                 {
int ip=SendMessage(hskladiste1,LVM_GETNEXTITEM,(WPARAM)-1,(LPARAM)LVNI_SELECTED);
...
//int i = ListView_GetNextItem(hskladiste, -1, LVNI_SELECTED);
int ip=SendMessage(hskladiste1,LVM_GETNEXTITEM,(WPARAM)-1,(LPARAM)LVNI_SELECTED);
...
case WM_NOTIFY :
    ...
    if(((LPNMHDR)lParam)->code == NM_CLICK) {
        ...
        int index = SendMessage(hskladiste1, LVM_GETNEXTITEM, -1, LVNI_SELECTED);
