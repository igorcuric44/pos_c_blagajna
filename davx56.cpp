...
#define zBUTTON1 12001
#define zBUTTON2 12002
#define zIME 12003
#define zZAPORKA 12004

HWND zButton1,zButton2,zIme,zZaporka;
...
LRESULT CALLBACK prviprozor(HWND, UINT , WPARAM , LPARAM );
...
HWND hskladiste;
HWND prvi,drugi,treci,cetvrti,peti,sesti,sedmi,osmi,deveti,deseti,jedanaesti;
    ...
    LPSTR lpCmdLine, int nCmdShow)
{
	...
	wc.lpszClassName = "zaporka";
	wc.lpfnWndProc = (WNDPROC)prviprozor;
     	RegisterClassEx(&wc);
...
case WM_COMMAND:
         {
             if(HIWORD(wParam) == BN_CLICKED && LOWORD(wParam) ==hBUTTON1)   
                 {        
                prvi = CreateWindowEx(0,"zaporka","ZAPORKA",WS_OVERLAPPED | WS_SYSMENU,CW_USEDEFAULT,CW_USEDEFAULT,300,250,0,0,hins,0); 
                ShowWindow(prvi,SW_SHOW);
		  UpdateWindow(prvi);
...
LRESULT CALLBACK prviprozor(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
    ...
    case WM_CREATE:
{
CreateWindowEx(0,"Static","Ime",WS_CHILD | WS_VISIBLE| ES_CENTER,20,10,80,30,hwnd,0,hins,NULL);
zIme=CreateWindowEx(0,"Edit","Ime",WS_CHILD | WS_VISIBLE| WS_BORDER,140,10,130,30,hwnd,(HMENU)zIME,hins,NULL);
CreateWindowEx(0,"Static","Zaporka",WS_CHILD | WS_VISIBLE| ES_CENTER,20,50,80,30,hwnd,0,hins,NULL);
zZaporka=CreateWindowEx(0,"Edit","Zaporka",WS_CHILD | WS_VISIBLE| WS_BORDER|ES_PASSWORD,140,50,130,30,hwnd,(HMENU)zZAPORKA,hins,NULL);




zButton1 = CreateWindowEx(BS_PUSHBUTTON,"button","POTVRDI",
WS_CHILD | WS_VISIBLE,60,30,180,30,hwnd,(HMENU)zBUTTON1,hins,NULL);
                                   
 zButton2 = CreateWindowEx(BS_PUSHBUTTON,
                                   "button",
                                   "ODUSTANI",
                                   WS_CHILD | WS_VISIBLE,
                                   60,70,180,30,
...
case WM_SIZE:
        ...
        MoveWindow(zButton1,  40, HIWORD(lParam)-50, 80, 30, TRUE);
        MoveWindow(zButton2, LOWORD(lParam)-120, HIWORD(lParam)-50, 80, 30, TRUE);
...
case WM_COMMAND:
		...
		if(HIWORD(wParam) == BN_CLICKED && LOWORD(wParam) ==zBUTTON1)   
                 {


		string Ime,Zaporka;
		
		GetDlgItemString(hwnd,zIME,Ime);
		GetDlgItemString(hwnd,zZAPORKA,Zaporka);

cout<<Ime<<" "<<Zaporka<<endl;
...
stringstream sql;
sql<<"select * from djelatnici where ime='"+Ime+"'";
...
rc=sqlite3_step(res);
if(Ime==to_string(sqlite3_column_text(res, 1)) && Zaporka==to_string(sqlite3_column_text(res, 4)))
{
...
{
MessageBox(hwnd,"Pogresna zaporka","UNOS ZAPORKE",MB_OK);
sqlite3_finalize(res);
sqlite3_close(db);
DestroyWindow(prvi);
...
if(HIWORD(wParam) == BN_CLICKED && LOWORD(wParam) ==zBUTTON2)   
                 {
DestroyWindow(prvi);
EnableWindow (hButton1, TRUE);
}
...
strftime(datum1, 64, "%Y", localtime(&t));
...
sql2<<"select count(*) from racun;";
rc=sqlite3_prepare(db,sql2.str().c_str(),-1,&res,NULL);
rc=sqlite3_step(res);

int ixi=sqlite3_column_int(res,0);
ixi++;

sql2.clear();
sql2.str(string());

sql2<<"insert into racun (id_racun,vrijeme,datum,racun) values ("<<ixi<<",";
sql2<<"'"<<vrijeme<<"','"<<datum<<"',"<<suma<<");"; 
rc=sqlite3_prepare(db,sql2.str().c_str(),-1,&res,NULL);
rc=sqlite3_step(res);

char znakk1[80];
sprintf(znakk1,"%04d",ixi);
string stda=to_string(datum1).substr(2,4);
stda=to_string(znakk1)+"/"+stda;

SetDlgItemText(hwnd, hbRACUN,stda.c_str());

stringstream sql1;

sql1<<"select * from poduzece;";
rc=sqlite3_prepare(db,sql1.str().c_str(),-1,&res,NULL);
rc=sqlite3_step(res);

string Poduzece,Ime,Prezime,Adresa,Mjesto,Oib,Telefon;
Poduzece=LPSTR(sqlite3_column_text(res,0));
Ime=LPSTR(sqlite3_column_text(res,1));
Prezime=LPSTR(sqlite3_column_text(res,2));
Adresa=LPSTR(sqlite3_column_text(res,3));
Mjesto=LPSTR(sqlite3_column_text(res,4));
Oib=LPSTR(sqlite3_column_text(res,5));
Telefon=LPSTR(sqlite3_column_text(res,6));

string adresax=Adresa+", "+Mjesto;
string imex="vl."+Ime+" "+Prezime;
string oibx="OIB :"+Oib;
string telx="tel. "+Telefon;
...
TextOut(hPrinter, 0, 0,Poduzece.c_str(),strlen(Poduzece.c_str()));
TextOut(hPrinter, 0, 30,adresax.c_str(),strlen(adresax.c_str()));
TextOut(hPrinter, 0, 60,imex.c_str(),strlen(imex.c_str()));
TextOut(hPrinter, 0, 90,oibx.c_str(),strlen(oibx.c_str()));
TextOut(hPrinter, 0, 120,telx.c_str(),strlen(telx.c_str()));
TextOut(hPrinter, 0, 180,"Datum :",strlen("Datum :"));
TextOut(hPrinter,150, 180,to_string(datum).c_str(),strlen(to_string(datum).c_str()));
TextOut(hPrinter, 0, 210,"Vrijeme :",strlen("Vrijeme :"));
TextOut(hPrinter,150, 210,to_string(vrijeme).c_str(),strlen(to_string(vrijeme).c_str()));
TextOut(hPrinter,0, 240,"Operator",strlen("Operator"));
TextOut(hPrinter,150, 240,ope.c_str(),strlen(ope.c_str()));
TextOut(hPrinter,0, 270,"Racun :",strlen("Racun :"));
TextOut(hPrinter,150, 270,stda.c_str(),strlen(stda.c_str()));
TextOut(hPrinter,0, 300,"Usluga",strlen("Usluga"));
TextOut(hPrinter,250, 300,"Kolicina",strlen("Kolicina"));
TextOut(hPrinter,0, 330,"Mjera",strlen("Mjera"));
TextOut(hPrinter,90, 330,"Cijena",strlen("Cijena"));
TextOut(hPrinter,200, 330,"Pdv",strlen("Pdv"));
TextOut(hPrinter,290, 330,"Iznos",strlen("Iznos"));
TextOut(hPrinter, 0, 360,"----------------------------",strlen("----------------------------"));;
int x=390;
for(i=0;i<iItem;i++){
for(int j=0;j<=6;j++){
cout<<ary[i][j]<<" ";
...
SelectObject (hPrinter, DisplayFont);
TextOut(hPrinter,0,x+70,"UKUPNO",strlen("UKUPNO"));
char znak[256];
sprintf(znak,"%05.2f",suma);
