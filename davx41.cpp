...
string sql2= "select id1,id2,mjera1,mjera2 from sifrarnik where id="+to_string(Temp11[0])+";";
cout<<sql2<<endl;
rc = sqlite3_prepare(db, sql2.c_str(), -1, &res, 0);
int step = sqlite3_step(res);

int id1=sqlite3_column_int(res,0);
int id2=sqlite3_column_int(res,1);
double mjera1=sqlite3_column_double(res,2);
double mjera2=sqlite3_column_double(res,3);
...
if(id1!=0){
sql1="select kolicina from skladiste where id in ";
sql1+= "(select id1 from sifrarnik where id="+to_string(Temp11[0])+");";
cout<<sql1<<endl;
rc = sqlite3_prepare(db, sql1.c_str(), -1, &res, 0);
step = sqlite3_step(res);
kolicinaxx=atof(Temp11[2])*mjera1;
kolicina=sqlite3_column_double(res,0);
kolicina=kolicina-kolicinaxx;
cout<<"---"<<endl;
cout<<kolicina<<endl;
cout<<kolicinaxx<<endl;
cout<<"---"<<endl;
sql1= "update skladiste set kolicina="+to_string(kolicina)+" where id="+to_string(id1);
cout<<sql1<<endl;
rc = sqlite3_prepare(db, sql1.c_str(), -1, &res, 0);
step = sqlite3_step(res);
}
...
if(id2!=0){
sql1="select kolicina from skladiste where id in ";
sql1+= "(select id2 from sifrarnik where id="+to_string(Temp11[0])+");";
cout<<sql1<<endl;
rc = sqlite3_prepare(db, sql1.c_str(), -1, &res, 0);
step = sqlite3_step(res);
...
cout<<kolicinaxx<<endl;
cout<<"---"<<endl;
sql1= "update skladiste set kolicina="+to_string(kolicina)+" where id="+to_string(id2);
cout<<sql1<<endl;
rc = sqlite3_prepare(db, sql1.c_str(), -1, &res, 0);
step = sqlite3_step(res);
}
...
cout<<Temp11[0]<<" "<<Temp11[2]<<" "<<Temp11[3]<<" "<<kolicinaxx<<endl;
