...
HWND hskladiste;
HWND drugi,treci,cetvrti,peti,sesti,sedmi,osmi;
...
LvItem1.iSubItem=0;
LvItem1.pszText=LPSTR(sqlite3_column_text(res, 1));

SendMessage(hblagajna,LVM_INSERTITEM,0,(LPARAM)&LvItem1);  

LvItem1.iSubItem=1;         // Put in first coluom
LvItem1.pszText=LPSTR("Artikl1");   // Text to display (can be from a char variable) (Items)
...
SetDlgItemText(hwnd, hbUKUPNO,LPSTR(znak));
	...
	SetDlgItemText(hwnd, hbDATUM,LPSTR(datum));
	SetDlgItemText(hwnd, hbVRIJEME,LPSTR(vrijeme));
	SetDlgItemText(hwnd, hbUKUPNO,LPSTR("0.0"));
	suma=0;        
        cout<<"space"<<endl;
...
{
CreateWindowEx(0,"Static","Id",WS_CHILD | WS_VISIBLE,20,10,80,30,hwnd,0,hins,NULL);
uaId=CreateWindowEx(0,"Edit","Id",WS_CHILD | WS_VISIBLE| WS_BORDER,140,10,130,30,hwnd,(HMENU)uaID,hins,NULL);
