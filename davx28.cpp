...
drugi = CreateWindowEx(0,"blagajna","BLAGAJNA",WS_OVERLAPPED | WS_SYSMENU,CW_USEDEFAULT,CW_USEDEFAULT,800,700,0,0,hInstance,0);
...
OldButtonProc = (WNDPROC)SetWindowLongPtr(hblagajna, GWLP_WNDPROC, (  LONG_PTR)ButtonProc);
...
LRESULT CALLBACK drugiprozor(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
 ...
 if (msg == WM_KILLFOCUS) {
	...
	case WM_CREATE:
	{
CreateWindowEx(0,"Static","Datum",WS_CHILD | WS_VISIBLE,20,10,80,30,hwnd,0,hins,NULL);
CreateWindowEx(0,"Static","Vrijeme",WS_CHILD | WS_VISIBLE,20,60,80,30,hwnd,0,hins,NULL);
CreateWindowEx(0,"Static","Racun",WS_CHILD | WS_VISIBLE,20,110,80,30,hwnd,0,hins,NULL);
CreateWindowEx(0,"Static","Operator",WS_CHILD | WS_VISIBLE,20,160,80,30,hwnd,0,hins,NULL);
...
LvCol.cx=0x75;
LvCol.pszText=LPSTR("Artikl");                     // First Header
SendMessage(hblagajna,LVM_INSERTCOLUMN,0,(LPARAM)&LvCol);
LvCol.pszText=LPSTR("Kolicina");
SendMessage(hblagajna,LVM_INSERTCOLUMN,1,(LPARAM)&LvCol); // ...
LvCol.pszText=LPSTR("Mjera");                       //
SendMessage(hblagajna,LVM_INSERTCOLUMN,2,(LPARAM)&LvCol); //
LvCol.pszText=LPSTR("Cijena");                       //
SendMessage(hblagajna,LVM_INSERTCOLUMN,3,(LPARAM)&LvCol); //
LvCol.pszText=LPSTR("Pdv");                       //
SendMessage(hblagajna,LVM_INSERTCOLUMN,4,(LPARAM)&LvCol); //
LvCol.pszText=LPSTR("Iznos");                       //
SendMessage(hblagajna,LVM_INSERTCOLUMN,5,(LPARAM)&LvCol);                       //
	...
	case WM_SIZE:
         {
          ...
          MoveWindow(hblagajna, 20, 180, LOWORD(lParam)-40, HIWORD(lParam)-220, TRUE);    
