...
case WM_NOTIFY:
{
if(((LPNMHDR)lParam)->code == NM_CLICK && LOWORD(wParam) ==HBLAGAJNA)   
                 { 
SetFocus(hwnd);
}
}
...
LvItem1.iSubItem=0;
LvItem1.pszText=LPSTR("Artikl");

SendMessage(hblagajna,LVM_INSERTITEM,0,(LPARAM)&LvItem1);  

LvItem1.iSubItem=1;         // Put in first coluom
LvItem1.pszText=LPSTR("Artikl");   // Text to display (can be from a char variable) (Items)
