...
HWND hblagajna,hbButton1,hbButton2,hbButton3,hbUkupno;
...
SendMessage(hblagajna,LVM_SETITEM,0,(LPARAM)&LvItem1);
LvItem1.iSubItem=5;         // Put in first coluom
LvItem1.pszText=LPSTR(to_string(s*cijena).c_str());
suma+=cijena;
SendMessage(hblagajna,LVM_SETITEM,0,(LPARAM)&LvItem1);
SetDlgItemText(hwnd, hbUKUPNO,LPSTR(to_string(suma).c_str()));

}	
        if((wParam)==32){  
	SendMessage(hblagajna,LVM_DELETEALLITEMS,0,0);
	SetDlgItemText(hwnd, hbUKUPNO,LPSTR(0));
	suma=0;        
        cout<<"space"<<endl;
        }
